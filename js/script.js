const mockCarros = [
  { id: 1, marca: "BMW", placa: "ABC-1234", cor: "Cinza", status: "Disponível", quilometragem: 15000, imagem: "carro1.png" },
  { id: 2, marca: "Fiat Argo", placa: "DEF-5678", cor: "Vermelho", status: "Alugado", quilometragem: 32000, imagem: "carro2.png" },
...
const mockManutencoes = [
  { entrada: "2025-09-01", saida: "2025-09-07", motivo: "Troca de motor", custo: 4500 },
];
...
function carregarCarros() {
  ...
  content.innerHTML = `
    <div class="grid grid-cols-1 md:grid-cols-3 gap-4" id="carros"></div>
  `;

  const container = document.getElementById("carros");
  mockCarros.forEach(c => {
...
document.addEventListener("DOMContentLoaded", () => {
  ...
  buttons.forEach(btn => {
    btn.addEventListener("click", () => {
      ...
      if (tab === "carros") carregarCarros();
      if (tab === "aluguéis") carregarAlugueis();
      if (tab === "infrações") carregarInfracoes();
      if (tab === "clientes") carregarClientes();
      if (tab === "manutenções") carregarManutencoes();
    });
  });
